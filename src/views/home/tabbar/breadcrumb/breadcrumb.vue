<template>
      <div class="tabbar_left">
        ...
        <el-breadcrumb-item :to="{ path: item.path }"  v-for="(item,index) in $route.matched" :key="index" v-show="!(item.path=='/home')"  style="line-height: 14px;">
            <el-icon  :class="item.meta.icon" style="margin-right: 5px;" size="18"> 
                 <component :is="item.meta.icon"></component>    
            </el-icon>
            <span style="display: inline-block;margin-bottom: 6px;">{{ item.meta.title }}</span>
        </el-breadcrumb-item>
